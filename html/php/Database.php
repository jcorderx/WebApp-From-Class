...
class Database {
    ...
    public function __construct() {
        if (self::$sqlConn === NULL) {
            self::$sqlConn = new mysqli(ini_get("mysqli.default_host"), ini_get("mysqli.default_user"), ini_get("mysqli.default_pw"), "test");
    ...
    */
    ...
    public function GetNotificationsFromQuery($query) {
        ...
        if ($qresult) {
            ...
            while ($row = $qresult->fetch_assoc()) {
                $targetAccType = $row["targetAccType"];
                $id = $row["id"];
                ...
                $hasBeenSeen = $row["hasBeenSeen"] == 0 ? false : true;

                $n = new Notification(Notification::$stringToAccType[$targetAccType], -1);
                $n->InitializeID($id);
                $n->InitializeHasBeenSeen($hasBeenSeen);
                $n->InitializeTimestamp($dateSent);
                $n->InitializeTargetID($driverId, $sponsorId, $adminId);
                $n->SetText($alertText);
