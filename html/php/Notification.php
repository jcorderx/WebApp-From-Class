...
class Notification {
    ...
    public static $accTypeToString = [
        UserAccount::DRIVER_ACCOUNT => "DRIVER",
        UserAccount::SPONSOR_ACCOUNT => "SPONSOR",
        UserAccount::ADMIN_ACCOUNT => "ADMIN",
    ];

    public static $stringToAccType = [
        "DRIVER" => UserAccount::DRIVER_ACCOUNT,
        "SPONSOR" => UserAccount::SPONSOR_ACCOUNT,
        "ADMIN" => UserAccount::ADMIN_ACCOUNT
    ];
    ...
    public function InitializeID($id) {$this->id = $id;}
    public function InitializeTimestamp($date) {$this->dateSent = $date;}
    public function InitializeHasBeenSeen($b) {$this->hasBeenSeen = $b;}
    public function InitializeTargetID($did, $sid, $aid) {
        switch ($this->targetAccType) {
            case UserAccount::DRIVER_ACCOUNT: $this->driverId = $did; break;
            case UserAccount::SPONSOR_ACCOUNT: $this->sponsorId = $sid; break;
            case UserAccount::ADMIN_ACCOUNT: $this->adminId = $aid; break;
        }
    }
    ...
    public function Post() {
        ...
        $accType = $this->db->sql->real_escape_string(self::$accTypeToString[$this->targetAccType]);
