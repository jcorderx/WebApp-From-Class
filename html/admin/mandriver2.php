...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    <form action="mandriver2.php" method="post" style="border:1px solid #ccc">
      <div class="container">
        ...
        <br>
        <label for="newPassword"><b>New Password (leave blank to stay the same):</b></label>
        <input type="password" class="form-control" placeholder="New User Password" name="newPassword">
        <br>
...
<?php
    ...
    if(isset($_POST["delete"]) && $userDr){
      if(isset($_POST["psw"])){
        ...
        $id = $user->GetID();
        $queryStr = "DELETE FROM Drivers WHERE id={$id};";
        $db->sql->query($queryStr);
