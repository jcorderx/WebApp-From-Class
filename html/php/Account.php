...
abstract class UserAccount {
    ...
    */
    ...
    /**
     * Get all Notifications for this user which have yet to be seen,
     * and mark them as seen.
     * 
     * RETURNS: The Notifications.
     */
    public function GetNotifications() {
        // mapping from account type to ID column name
        $idNames = [
            UserAccount::DRIVER_ACCOUNT => "driverId",
            UserAccount::SPONSOR_ACCOUNT => "sponsorId",
            UserAccount::ADMIN_ACCOUNT => "adminId"
        ];

        // get unseen notifications for this user
        $idName = $idNames[$this->GetAccountType()];
        $ns = $this->db->GetNotificationsFromQuery("SELECT * FROM AutomaticAlerts WHERE {$idName}={$this->GetID()} ORDER BY dateSent DESC LIMIT 25;");
...
class SponsorAccount extends UserAccount {
    ...
    public function GetDrivers() {
        return $this->db->LoadDriversFromQuery("SELECT * FROM Drivers WHERE id IN (SELECT driverId FROM DriverSponsorRelations WHERE sponsorId={$this->GetID()});");
    }
...
class DriverAccount extends UserAccount {
    ...
    /**
     * Purchase the given item under the given sponsor.
     *      $catalogItem, CatalogItem : The item to purchase.
     *      $sponsor, SponsorAccount : The sponsor to purchase from.
     * 
     * RETURNS: true on success, false on failure. False is returned
     * if the driver doesn't have enough credits or if the given sponsor
     * doesn't have this driver.
     */
    public function PurchaseItem($catalogItem, $sponsor) {
        $this->LoadSponsorIdsAndCredits();

        if (!in_array($sponsor->GetID(), $this->sponsorIds)) {
            return false;
        }

        $credits = $this->credits[$sponsor->GetID()];
        $price = $catalogItem->GetCreditPriceBySponsor($sponsor, $this);
        ...
        if ($credits < $price) {
            ...
            // 1. Deduct the points.
            // 2. Add to PurchaseHistory table.
            // 3. Send Notification to driver summarizing the order.

            $credits -= $price;
            $this->db->sql->query("UPDATE DriverSponsorRelations SET credits={$credits} WHERE sponsorId={$sponsor->GetID()} AND driverId={$this->GetID()};");
            $this->db->sql->query("INSERT INTO PurchaseHistory (driverId,itemId,sponsorId,price) VALUES ({$this->GetID()},{$catalogItem->GetID()},{$sponsor->GetID()},{$price});");

            $n = new Notification(UserAccount::DRIVER_ACCOUNT, $this->GetID());
            $n->SetText("You purchased item {$catalogItem->title} for {$price} credits, leaving you with {$credits} credits.");
            $n->Post();
        }
    ...
    public function GetPurchaseHistory() {
        ...
        $qresult = $this->db->sql->query("SELECT * FROM PurchaseHistory WHERE driverId={$this->GetID()};");

        if ($qresult) {
            while ($row = $qresult->fetch_assoc()) {
                $pitem = new PurchasedItem();
                $pitem->date = $row["purchaseTime"];
                $pitem->price = $row["price"]; // in credits
                $pitem->item = null;

                $catalogItem = $this->db->GetCatalogItemsFromQuery("SELECT * FROM CatalogItems WHERE id={$row['itemId']}");

                if (count($catalogItem) > 0) {
                    $pitem->item = $catalogItem[0];
                }

                $pitem->sponsor = $this->db->LoadUserFromId(UserAccount::SPONSOR_ACCOUNT, $row["sponsorId"]);
