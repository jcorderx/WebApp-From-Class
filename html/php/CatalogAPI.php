...
class eBayAPI {
    ...
    public static function GetSingleItem($itemId) {
        $apicall = self::GetShoppingAPICallTemplate("GetSingleItem");
    ...
    public static function GetCatalogItemFromShoppingJSON($json) {
        $decoded = json_decode($json, true);
        $itemJSON = $decoded["Item"];

        $citem = new CatalogItem();
        $citem->InitializeID($itemJSON["ItemID"]);

        $citem->title="";
        $citem->location="";
        $citem->viewItemURL="";
        $citem->imageURL="";
        $citem->shippingCost=0.0;
        $citem->currentPrice=0.0;
        $citem->conditionDisplayName="";
        $citem->buyItNowAvailable=true;
        $citem->startTime="";
        $citem->endTime="";
        $citem->categoryId="";
        $citem->categoryName="";

        $citem->title = $itemJSON["Title"] ?? "";
        $citem->location = $itemJSON["Location"] ?? "";
        $citem->viewItemURL = $itemJSON["ViewItemURLForNaturalSearch"] ?? "";
        $citem->imageURL = $itemJSON["PictureURL"][0] ?? "";
        $citem->categoryId = $itemJSON["PrimaryCategoryID"] ?? "";
        $citem->categoryName = $itemJSON["PrimaryCategoryName"] ?? "";
        $citem->currentPrice = $itemJSON["ConvertedCurrentPrice"]["Value"] ?? "";
        $citem->conditionDisplayName = $itemJSON["ConditionDisplayName"] ?? "Unknown";

        return $citem;
    }

    public static function GetItemIDFromURL($url) {
        //https://www.ebay.com/itm/Bose-QuietComfort-35-Series-I-Wireless-Headphones-Factory-Renewed/163543329737?_trkparms=5373%3A0%7C5374%3AFeatured
        $matches = [];
