...
//This query is used to get an item count
$queryString = "SELECT * FROM cart WHERE did={$user->GetID()};";
$result = $db->sql->query($queryString);
...
foreach ($items as $item) {
  $total += $item->GetCreditPriceBySponsor($sponsor, $user);
}
...
-->
          ...
          <a class="btn" href="/driver/cartPurchase.php?sponsorId=<?= $sponsor->GetID() ?>">Purchase</a>
        </form>
      </div>
    </div>
    <div class="col-25">
      <div class="container">
        <h4>Cart <span class="price" style="color:black"><i class="fa fa-shopping-cart"></i> <b><?= count($items) ?></b></span></h4>

        <?php foreach($items as $item): ?>

        <p>
          <a href="<?= $item->viewItemURL ?>"><?= FormatTitle($item->title) ?> </a>
          <span class="price">
            <?= $item->GetCreditPriceBySponsor($sponsor, $user) ?>
          </span>
        </p>
        ...
        <hr>
        <p>Total <span class="price" style="color:black"><b><?= $total ?></b></span> Points</p>
      </div>
    </div>
  </div>
