...
class CatalogItem {
    ...
    public function GetCreditPriceBySponsor($sponsor, $driver) {
      $pratio = 1;

      $db = new Database();

      // get the pratio from the database
      $qresult = $db->sql->query("SELECT pRatio FROM DriverSponsorRelations WHERE sponsorId={$sponsor->GetID()} AND driverId={$driver->GetID()};");
      if ($qresult && $qresult->num_rows > 0) {
        $pratio = $qresult->fetch_assoc()["pRatio"];
        $qresult->free();
      }

      return ceil($this->currentPrice / $pratio) + ceil($this->shippingCost / $pratio);
    }
}
...
class Catalog {
  ...
  private function RegenCatalogByList_() {
    foreach ($this->GetItems() as $item) {
      $json = eBayAPI::GetSingleItem($item->GetID());
      $item = eBayAPI::GetCatalogItemFromShoppingJSON($json);
      $this->db->AddOrUpdateCatalogItem($item);
    }
  }
