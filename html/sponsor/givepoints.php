...
if (!empty($_POST["points"]) && $_POST["points"] !== 0) {
	$points = $_POST["points"];
    ...
    $n = new Notification(UserAccount::DRIVER_ACCOUNT, $did);

    if ($points > 0) {
        $n->SetText("You gained {$points} points with {$user->GetUsername()}!");
    }
    else if ($points < 0) {
        $n->SetText("You lost {$points} points with {$user->GetUsername()}.");
    }
    $n->Post();
}
